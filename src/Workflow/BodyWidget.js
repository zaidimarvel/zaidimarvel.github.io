...
export class BodyWidget extends React.Component {
    ...
    //    console.log(this.props.app.getActiveDiagram());
            ...
            nodesUpdated: function(e) {
              ...
              if(e.isCreated){
                let newNode =  {
                  ...
                  ports: JSON.parse(JSON.stringify(e.node.ports, that.getCircularReplacer())),
                ...
                event.link.addListener({
                    selectionChanged: function(d) {
                      ...
                      targetPortChanged: function(d) {
                        ...
                        if(event.link.targetPort){
                          if(event.link.targetPort.name == event.link.sourcePort.name){
                            ...
                            let results =event.link.sourcePort.parent.printResults(event.link.sourcePort.parent,event.link.sourcePort.label)
                                  let selected_node = {
                          ...
                          if(Object.keys(targetObject).length > link.targetPort.parent.properties.maxLinks){
                            event.link.remove()
                          }else{
                            // console.log(d);
                          let newConnection =  {
                            stamp: link.sourcePort.parent.id + '' + link.targetPort.parent.id,
                            link_id: link.id,
                            source: {
                          ...
                          let tarNode = that.props.app.diagramEngine.getDiagramModel().getNode(link.targetPort.parent.id)
                          tarNode.properties.headers = link.sourcePort.parent.properties.headers
                      ...
                      entityRemoved: function(d) {
                          ...
                          let data = that.state.connections.filter(link => ( link.link_id != d.entity.id));
        ...
        updateNode(node) {
          ...
          let currNode = this.props.app.diagramEngine.getDiagramModel().getNode(this.state.selected_node.id)
        ...
        saveFileConf = (e, node) => {
          ...
          let currNode = this.props.app.diagramEngine.getDiagramModel().getNode(node.id)
          let that = this
        ...
        runWorkflow = (e) => {
          ...
          let inputNodes = this.state.nodes.filter((node) =>  node.properties.type == 'single')
          let cnt = 1
          let newConns = []
          ...
          inputNodes.forEach(node => {
            ...
            if(node.nodeType == 'File'){
              let workingNode = that.props.app.diagramEngine.getDiagramModel().getNode(node.id)
      ...
      <div className="text-left">
        ...
        <Form>
          <Row>
            <Col md="12">
              <FormGroup>
              <FilePondComp node={node} model={this.props.app.diagramEngine.getDiagramModel()} updateNode={this.updateNode} file_name={node.properties.file}/>
                
                {node.properties.sheets == null ? '': 
                <div>
                  <h6 className="heading mt-2">Select Sheet</h6>
                  <Input type="select" name="sheet" 
                  value={(node.properties.selected_sheet == null || node.properties.selected_sheet == '') ? 'Default':node.properties.selected_sheet }
        ...
        {(node.properties.selected_sheet == null || node.properties.start_index == null) ? '':
          <Button
          aria-pressed={true}
          className="active"
          color="secondary"
       ...
       <p>
       {/* <span id="coderea">{ JSON.stringify(node) }</span> */}
       </p>
        ...
        toggleWorkflow(workflow){
          if(this.state.activeTab !== workflow) this.setState({activeTab : workflow});
        }
            ...
            <Card>
              <CardHeader className="bg-transparent pt-2 pb-2">
                <Row className="align-items-center">
                  <div className="col">
                   ...
                   <Button
                      ...
                      color= {this.state.runningState == 'run'? 'success':'danger'}
                ...
                <Row className="align-items-left">
                  ...
                  <TrayItemWidget model={{ type: "File" }} name="Input"  icon="file-alt" color="#009688" /> 
                  <TrayItemWidget model={{ type: "Output" }} name="Output"  icon="file-export" color="#009688" /> 
                  <TrayItemWidget model={{ type: "Filter" }} name="Filter"  icon="filter" color="linear-gradient(to right, #de6262, #ffb88c" /> 
                  <TrayItemWidget model={{ type: "Formula" }} name="Formula"  icon="square-root-alt" color="#02aab0" /> 
                  <TrayItemWidget model={{ type: "Union" }} name="Union"  icon="layer-group" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "Unique" }} name="Unique"  icon="code-branch" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "Peek" }} name="Peek"  icon="eye" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "Select" }} name="Select"  icon="tasks" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "Sort" }} name="Sort"  icon="sort" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "Cleanup" }} name="Cleanup"  icon="broom" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "Group" }} name="GroupBy"  icon="object-group" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "Sample" }} name="Sample"  icon="vials" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "Replace" }} name="Replace"  icon="search" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "TextToColumn" }} name="TextToColumn"  icon="columns" color="#7b4397" /> 
                  <TrayItemWidget model={{ type: "Join" }} name="Join"  icon="columns" color="#7b4397" /> 
                  
                    </TrayWidget>
                  </div>
                  </Row>
                  </CardHeader>
                  <CardBody className="p--1" style={{padding: '0.5em', borderRadius: '20px'}}>
                    <div className="body">
                <div className="content">
                    ...
                    >
                        {/* smartRouting={true}  */}
                        <DiagramWidget className="srd-demo-canvas" deleteKeys={[46]} diagramEngine={this.props.app.getDiagramEngine()} allowLooseLinks={false}  maxNumberPointsPerLink="0"  />
                    </div>
                </div>
            </div>
            </CardBody>
              ...
              <CardHeader className="bg-transparent">
                <Row className="align-items-center">
                  <div className="col">
                    ...
                    <h2 className="mb-0">{ this.state.selected_node == null ? "Workspace" : this.state.selected_node.properties.name } - Properties</h2>
                  </div>
                </Row>
              </CardHeader>
              <CardBody>
